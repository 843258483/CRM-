		...
		while (i--) {
			...
			for (j = 0; j < length; j++) {
				...
				if (distanceOption > 0) {
					...
					// to fall back to the resource position
					y = slot.y;
					if ((resourceY > y && slots[slotIndex + 1] !== null) ||
							(resourceY < y &&  slots[slotIndex - 1] !== null)) {
						y = mathMin(mathMax(0, resourceY), chart.plotHeight);
					}
				...
				// get the x - use the resource x position for first and last slot, to prevent the top
				// and botton slice connectors from touching each other on either side
				x = options.justify ?
					seriesCenter[0] + (i ? -1 : 1) * (radius + distanceOption) :
					series.getX(y === centerY - radius - distanceOption || y === centerY + radius + distanceOption ? resourceY : y, i);
